  ...
  await new Promise((resolve, reject) => {
    operation.attempt(async () => {
      try {
        ...
        log(config, `[${new Date().toISOString()}] 上传成功: ${key}`);
        resolve();
      } catch (error) {
...
const deleteCosFiles = async (cos, config, keys) => {
  ...
  log(config, `[${new Date().toISOString()}] 删除 ${keys.length} 个远程文件`);
};
...
const purgeCdnCache = async (cdn, config, urls) => {
  ...
  for (let i = 0; i < urls.length; i += batchSize) {
    ...
    log(config, `[${new Date().toISOString()}] 刷新 ${batchUrls.length} 个 CDN URL`);
  }
};
      ...
      await new Promise((resolve) => {
        operation.attempt(async () => {
          try {
            ...
            log(config, `[${new Date().toISOString()}] 清除缓存任务成功: Type=${task.type}, Targets=${task.targets}`);
            resolve();
          } catch (error) {
            if (operation.retry(error)) return;
  ...
  if (config.cdnDomains.length && changedFiles.length) {
    ...
    config.cdnDomains.forEach(({ domain, ignorePaths, ignoreExtensions }) => {
      changedFiles.forEach(file => {
        ...
        const urlPath = config.refreshIndexPage && urlFile.endsWith('/index.html')
    ...
    if (config.cache_type === 'cdn') {
      ...
      await purgeEdgeOneCache(edgeone, config, urls);
    }
  }

  log(config, `[${new Date().toISOString()}] 部署完成！`);
};
