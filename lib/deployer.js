...
const purgeEdgeOneCache = async (edgeone, urls) => {
  ...
  for (const mainDomain of mainDomains) {
    ...
    let zoneId;
    try {
      const response = await edgeone.DescribeZones({});
      const zone = (response?.ZoneList || []).find(z => z.Name === mainDomain || z.Name.endsWith(`.${mainDomain}`));
      zoneId = zone?.ZoneId;
      if (!zoneId) {
    ...
    let quota;
    try {
      const response = await edgeone.DescribeContentQuota({ ZoneId: zoneId });
      const purgeUrlQuota = (response?.QuotaList || []).find(q => q.Type === 'purge_url');
      quota = {
        batchLimit: purgeUrlQuota?.Batch || 500,
    ...
    const isFreePlan = quota.batchLimit === 500;
  ...
  if (config.cdnDomains.length && changedFiles.length) {
    ...
    config.cdnDomains.forEach(({ domain, ignorePaths, ignoreExtensions }) => {
      changedFiles.forEach(file => {
        ...
        const url = config.refreshIndexPage && file.endsWith('/index.html')
          ? `${domain}/${file.replace(/\/index\.html$/, '/')}`
          : `${domain}/${file}`;
        urls.push(url);
      });
    });
