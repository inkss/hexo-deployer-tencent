...
const validateConfig = (hexo) => {
  ...
  if (missing.length) {
    console.error(`缺少必要的 deploy 配置项: ${missing.join(', ')}`);
    return null;
  }

  required.forEach(item => {
    if (config[item] === `your_${item}`) {
      console.error(`未正确配置: ${item}`);
    }
  });
  ...
  cdnDomains.forEach((item, index) => {
    if (!item.domain) {
      console.error(`cdn_domains[${index}] 缺少 domain 字段`);
    } else if (!item.domain.startsWith('http://') && !item.domain.startsWith('https://')) {
      console.error(`cdn_domains[${index}] 的 domain 必须以 http:// 或 https:// 开头`);
    }
    if (item.ignore_paths && !Array.isArray(item.ignore_paths)) {
      console.error(`cdn_domains[${index}] 的 ignore_paths 必须是数组`);
    }
  });
...
const main = async (config, clients) => {
  ...
  if (config.cdnDomains.length && changedFiles.length) {
    const urls = [];
    config.cdnDomains.forEach(({ domain, ignorePaths }) => {
      changedFiles.forEach(file => {
        const shouldIgnore = ignorePaths.some(pathPrefix => {
